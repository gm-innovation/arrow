-- Function to notify technicians when a service order is created or updated
CREATE OR REPLACE FUNCTION notify_technicians_on_service_order_change()
RETURNS TRIGGER AS $$
DECLARE
...
-- Function to notify technician when assigned to a task
CREATE OR REPLACE FUNCTION notify_technician_on_task_assignment()
RETURNS TRIGGER AS $$
DECLARE
  v_user_id UUID;
  v_order_number TEXT;
...
-- Function to notify technicians when scheduled date changes
CREATE OR REPLACE FUNCTION notify_technicians_on_schedule_change()
RETURNS TRIGGER AS $$
DECLARE
  v_task_record RECORD;
  v_new_date TEXT;
...
DROP TRIGGER IF EXISTS trigger_notify_technicians_on_service_order_insert ON service_orders;
DROP TRIGGER IF EXISTS trigger_notify_technicians_on_service_order_update ON service_orders;
DROP TRIGGER IF EXISTS trigger_notify_technician_on_task_assignment ON tasks;
DROP TRIGGER IF EXISTS trigger_notify_technicians_on_schedule_change ON service_orders;

-- Create trigger for new service orders (after tasks are created)
CREATE TRIGGER trigger_notify_technicians_on_service_order_insert
AFTER INSERT ON service_orders
FOR EACH ROW
EXECUTE FUNCTION notify_technicians_on_service_order_change();

-- Create trigger for task assignments
CREATE TRIGGER trigger_notify_technician_on_task_assignment
AFTER INSERT OR UPDATE ON tasks
FOR EACH ROW
EXECUTE FUNCTION notify_technician_on_task_assignment();

-- Create trigger for schedule changes
CREATE TRIGGER trigger_notify_technicians_on_schedule_change
AFTER UPDATE ON service_orders
FOR EACH ROW
EXECUTE FUNCTION notify_technicians_on_schedule_change();